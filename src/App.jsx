...
// ─── Constants ───────────────────────────────────────────────────────────────────
const MAX_FAIL = 5;
const DOC_REF = doc(db, 'gratitude-coins', 'state'); // Firestore single document
...
// ─── Initial Firestore Document Structure ─────────────────────────────────────────
const INIT_STATE = {
...
export default function App() {
  ...
  const [adminPwErr, setAdminPwErr] = useState(''); // Changed to string
  ...
  const doAdminLogin = () => {
    if (appState.adminLocked) {
      setAdminPwErr('관리자 계정이 잠겼습니다. 비밀번호를 초기화해주세요.');
      return;
    }
    if (adminPw !== appState.adminPassword) {
      const fails = (appState.adminFailedAttempts ?? 0) + 1;
      const nowLock = fails >= MAX_FAIL;
      persist({ adminFailedAttempts: fails, adminLocked: nowLock });
      setAdminPwErr(nowLock ? `비밀번호 ${MAX_FAIL}회 오류. 계정이 잠겼습니다.` : `비밀번호 오류 (${MAX_FAIL - fails}회 남음)`);
      return;
    }
    persist({ adminFailedAttempts: 0, adminLocked: false });
  ...
  const doResetAdminPassword = () => {
    persist({ adminPassword: 'admin1234', adminFailedAttempts: 0, adminLocked: false });
    setAdminPw('');
    setAdminPwErr('');
    notify('관리자 비밀번호가 초기화되었습니다.');
  };
  ...
  const resetAllMembersAndData = () => {
    ...
    try {
      persist({
        ...INIT_STATE,
        adminPassword: appState.adminPassword, // Keep admin password
      });
      notify('모든 회원 및 관련 데이터가 초기화되었습니다.');
    } catch (error) {
      console.error("Error resetting member data:", error);
      ...
      {modal === 'adminLogin' && (
        <div style={S.overlay} onClick={closeModal}>
          <div style={S.card2} onClick={e => e.stopPropagation()}>
            ...
            {appState.adminLocked ? (
              <>
                <p style={S.errMsg}>관리자 계정이 잠겼습니다. 비밀번호를 초기화해주세요.</p>
                <button style={{ ...S.btn, background: 'linear-gradient(135deg,#f59e0b,#d97706)' }} onClick={doResetAdminPassword}>비밀번호 초기화</button>
              </>
            ) : (
              <>
                <Inp label="비밀번호" type="password" placeholder="관리자 비밀번호" value={adminPw} onChange={v => { setAdminPw(v); setAdminPwErr(''); }} onEnter={doAdminLogin} error={adminPwErr} />
              ...
              <>
                ...
                {adminTab === 'settings' && (
                  <div style={S.card}>
                    ...
                    <button style={{ ...S.btn, background: 'linear-gradient(135deg,#ef4444,#dc2626)' }} onClick={() => setModal('adminChangePw')}>🔑 관리자 비밀번호 변경하기</button>
                  </div>
                )}
              </>
            )}
